...
RUN test "x${PYTHON}" = "x3.9" && \
    dnf update -y && \
    dnf install -y which findutils gridsite libaio memcached httpd mod_ssl python3-pip python3-mod_wsgi python3-gfal2 \
            sqlite gcc python3-devel python3-kerberos krb5-devel libxml2-devel xmlsec1-devel xmlsec1-openssl-devel \
            libtool-ltdl-devel mariadb-connector-c-devel && \
