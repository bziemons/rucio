...
if [[ $RDBMS == "oracle" ]]; then
    ...
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_oracle.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_oracle.ini /opt/rucio/etc/alembic.ini
    ...
    docker run --name=activemq -d webcenter/activemq:latest
    docker run -d --link mysql5:mysql5 --link activemq:activemq --name=rucio $IMAGE
    date
    while ! docker exec mysql5 mysqladmin --user=root --password=secret ping 2>&1; do
        sleep 1
    done
    date
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_mysql5.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_mysql5.ini /opt/rucio/etc/alembic.ini
    docker exec -it rucio httpd -k restart
    ...
    docker run --name=activemq -d webcenter/activemq:latest
    docker run -d --link mysql8:mysql8 --link activemq:activemq --name=rucio $IMAGE
    date
    while ! docker exec mysql8 mysqladmin --user=root --password=secret ping 2>&1; do
        sleep 1
    done
    date
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_mysql8.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_mysql8.ini /opt/rucio/etc/alembic.ini
    docker exec -it rucio httpd -k restart

elif [[ $RDBMS == "postgres9" ]]; then
    docker run --name=postgres9 -e POSTGRES_PASSWORD=secret -d postgres:9 -c 'max_connections=300'
    docker run --name=activemq -d webcenter/activemq:latest
    docker run -d --link postgres9:postgres9 --link activemq:activemq --name=rucio $IMAGE
    date
    while ! docker exec postgres9 pg_isready 2>&1; do
        sleep 1
    done
    date
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_postgres9.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_postgres9.ini /opt/rucio/etc/alembic.ini
    docker exec -it rucio httpd -k restart

elif [[ $RDBMS == "postgres12" ]]; then
    docker run --name=postgres12 -e POSTGRES_PASSWORD=secret -d postgres:12 -c 'max_connections=300'
    docker run --name=activemq -d webcenter/activemq:latest
    docker run -d --link postgres12:postgres12 --link activemq:activemq --name=rucio $IMAGE
    date
    while ! docker exec postgres12 pg_isready 2>&1; do
        sleep 1
    done
    date
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_postgres12.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_postgres12.ini /opt/rucio/etc/alembic.ini
    docker exec -it rucio httpd -k restart

elif [[ $RDBMS == "sqlite" ]]; then
    docker run -d -p 443:443 --name=rucio $IMAGE
    docker exec -it rucio cp /opt/rucio/etc/docker/test/rucio_sqlite.cfg /opt/rucio/etc/rucio.cfg
    docker exec -it rucio cp /opt/rucio/etc/docker/test/alembic_sqlite.ini /opt/rucio/etc/alembic.ini
    docker exec -it rucio httpd -k restart
fi
