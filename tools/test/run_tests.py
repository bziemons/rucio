...
def main():
    ...
    if parallel:
        with multiprocessing.Pool(
            processes=min(int(os.environ.get('PARALLEL_AUTOTESTS_PROCNUM', 3)), len(cases)),
            maxtasksperchild=1,
        ) as prpool:
            prepared_tasks = [
                (_case, {'run_case_kwargs': gen_case_kwargs(_case), 'stdlog': logs_dir / f'log-{case_id(_case)}.txt'})
                for _case in cases
            ]
            tasks = [
                (_case, prpool.apply_async(run_case_logger, (), _casekwargs)) for _case, _casekwargs in prepared_tasks
            ]
